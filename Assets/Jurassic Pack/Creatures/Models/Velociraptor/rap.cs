...
public class rap : MonoBehaviour
{
	...
	const float MAXYAW=16, MAXPITCH=9, MAXCROUCH=2.5f, MAXANG=4, ANGT=0.15f;
	...
	void PlaySound(string name, int time)
	{
		if(time==shared.currframe && shared.lastframe!=shared.currframe)
		{
			...
			case "Food": source[0].pitch=Random.Range(3.0f, 3.5f); source[0].PlayOneShot(Swallow, 0.025f);
				shared.lastframe=shared.currframe; break;
			case "Sleep": source[0].pitch=Random.Range(3.0f, 3.5f); source[0].PlayOneShot(Idlecarn, 0.01f);
				shared.lastframe=shared.currframe; break;
	...
	void FixedUpdate()
	{
		...
		//Various
		else if(CurrAnm.IsName("Rap|IdleAtk3")) { shared.IsAttacking=true; transform.rotation*= Quaternion.Euler(0, anm.GetFloat("Turn")*MAXANG, 0); PlaySound("AtkA", 1); }
		...
		{ shared.IsAttacking=true; transform.rotation*= Quaternion.Euler(0, anm.GetFloat("Turn")*MAXANG, 0); PlaySound("AtkB", 2); PlaySound("Bite", 9); }
		...
		else if(CurrAnm.IsName("Rap|EatA")) { reset=true; PlaySound("Food", 1); }
		...
		else if(CurrAnm.IsName("Rap|Die-")) { reset=true; PlaySound("AtkA", 1);  shared.IsDead=false; }
	}
