...
public class ovi : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		//Various
		else if(CurrAnm.IsName("Ovi|IdleAtk3")) { shared.IsAttacking=true; transform.rotation*= Quaternion.Euler(0, anm.GetFloat("Turn")*MAXANG, 0); PlaySound("AtkB", 1); }
		...
		else if(CurrAnm.IsName("Ovi|EatA")) { reset=true; PlaySound("Food", 1); }
		...
		else if(CurrAnm.IsName("Ovi|Die-")) { reset=true; PlaySound("AtkA", 1);  shared.IsDead=false; }
	}
