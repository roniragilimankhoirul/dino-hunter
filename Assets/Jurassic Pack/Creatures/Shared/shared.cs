...
public class shared : MonoBehaviour 
{
  ...
  [HideInInspector] public enum TargetAction { None, Sleep, Eat, Drink };
	...
	[HideInInspector] public bool IsActive, IsVisible, IsDead, IsOnGround, IsOnWater, IsInWater, IsConstrained, OnLevitation,
	CanAttack, CanFly, CanSwim, IsAnphibian, DepthSwim, CanJump, IsCrouching, IsJumping, IsAttacking, HasSideAttack;
...
//CHANGE SKIN
	...
	void OnDrawGizmosSelected()
	{
		...
		if(rend[0].sharedMaterials[0].mainTexture!=skin[BodySkin.GetHashCode()] | rend[0].sharedMaterials[1].mainTexture!=eyes[EyesSkin.GetHashCode()])
		{ 
...
//COUNTERS
	void Update()
	{
		...
		if(rend[0].isVisible | rend[1].isVisible | rend[2].isVisible | (Camera.main.transform.position-transform.position).magnitude< 100) { IsVisible=true; IsActive=true; } else
		{ 
			IsVisible=false;
		...
		if(manager.UseManager && transform.gameObject==manager.creaturesList[manager.selected].gameObject && manager.CameraMode!=0)
		{
			...
			else if(Input.GetKey(KeyCode.F)) //Eat / Drink
			{
				...
				if(IsOnWater)
				{
					...
					if(Water<100) { behavior="Water"; Water+=0.05f; }
...
//TARGET LOOKING (Randomly look around and crouch or target looking)
	public void TargetLooking(float spineX, float spineY, float crouch)
	{
		...
		if(objectTGT)
		{
			if(behavior.EndsWith("Hunt") | behavior.Equals("Battle") |  behavior.EndsWith("Contest") | behavior.Equals("Food")  )
...
// MANAGE COLLISIONS, DAMAGES AND BLOOD FX
	public void ManageCollision(Collision col, float pitch_max, float crouch_max, AudioSource[] source, AudioClip pain, AudioClip Hit_jaw, AudioClip Hit_head, AudioClip Hit_tail)
	{
		...
		else if(col.transform.root.tag.Equals("Creature"))
		{
			...
			{
				if(otherCreature.scale.x>scale.x) 	{ behaviorCount=100;  behavior="ToFlee"; }
				else 
				{
	...
	public void GetGroundAlt(bool quadruped, float crouch)
	{
		...
		else
		{
			...
			else
			{
				...
				if(IsVisible && manager.UseIK) // (require "JP script extension" asset)
				{ manager.message=2; manager.UseIK=false; return; }
			}
		}

		if(IsInWater) posY=waterY-body.centerOfMass.y;
...
//***********************************************************************************************************************************************************************************************************
	...
	public void QuadIK(Transform RArm1, Transform RArm2, Transform RArm3, Transform LArm1, Transform LArm2, Transform LArm3, Transform RLeg1, Transform RLeg2, Transform RLeg3, Transform LLeg1, Transform LLeg2, Transform LLeg3, float FeetOffset)
	{ if(manager.UseIK) { manager.message=2; manager.UseIK=false; } }
	public void SmallBipedIK(Transform RLeg1, Transform RLeg2, Transform RLeg3, Transform RLeg4, Transform LLeg1, Transform LLeg2, Transform LLeg3, Transform LLeg4)
  { if(manager.UseIK) { manager.message=2; manager.UseIK=false; } }
	public void LargeBipedIK(Transform RLeg1, Transform RLeg2, Transform RLeg3, Transform RLeg4, Transform LLeg1, Transform LLeg2, Transform LLeg3, Transform LLeg4)
 	{ if(manager.UseIK) { manager.message=2; manager.UseIK=false; } }
	public void ConvexQuadIK(Transform RArm1, Transform RArm2, Transform RArm3, Transform LArm1, Transform LArm2, Transform LArm3, Transform RLeg1, Transform RLeg2, Transform RLeg3,Transform LLeg1, Transform LLeg2, Transform LLeg3)
	{ if(manager.UseIK) { manager.message=2; manager.UseIK=false; } }
	public void FlyingIK(Transform RArm1, Transform RArm2, Transform RArm3, Transform LArm1, Transform LArm2, Transform LArm3, Transform RLeg1, Transform RLeg2, Transform RLeg3, Transform LLeg1, Transform LLeg2, Transform LLeg3)
  { if(manager.UseIK) { manager.message=2; manager.UseIK=false; } }
//***********************************************************************************************************************************************************************************************************
//
